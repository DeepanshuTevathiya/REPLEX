...
class PushUpDetector(BaseExercise):
    ...
    def process(self, landmarks):
        ...
        if abs(hip_deviation) <= self.HIP_SAG_TOLERANCE:
            hip_status = "LEVEL"
        elif hip_deviation > self.HIP_SAG_TOLERANCE: # in Mediapipe upper portion in image => small value
            hip_status = "SAGGING"                   #               lower portion in image => larger value
        else:
