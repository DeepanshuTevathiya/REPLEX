...
def main():
    ...
    initial_session_defaults()    #session states

    if "voice_pipeline" in st.session_state:
        del st.session_state["voice_pipeline"]
    ...
    with st.sidebar:
        log_out_button = st.button("Log Out", key="log_out")
        if log_out_button:
        ...
        if not workout_started:
            exercise_type = st.selectbox("Exercise", options=EXERCISE_OPTIONS, key="plan_exercise")
            sets = st.number_input("Sets", min_value=0, max_value=20, key="plan_sets", step=1)
            reps = st.number_input("Reps per set", min_value=0, max_value=50, key="plan_reps", step=2)
            ...
            if start_session_button:
                ...
                if (sets and reps) == 0:
                    st.error("Please set the Sets or Reps!")
                    return
    ...
    if not workout_started:
        ...
        context = webrtc_streamer(
            ...
            video_processor_factory=VideoProcessor,   #process video frames
    ...
    if isinstance(user_id, int):
        ...
        if not df.empty:
            ...
            agg_df = (df.groupby(["Exercise", "Date"], as_index=False).agg({
                "Reps":"sum",
                "Sets":"sum",
                "Time (min)": "sum"
