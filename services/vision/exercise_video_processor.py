...
class VideoProcessor(VideoProcessorBase):
    ...
    def set_latest_metric(self, metrics):
        with self._lock:
    ...
    def _draw_curl_overlays(self, img, metrics):
        ...
        cv2.putText(
            img,
            f"SWING: {metrics['swing_status']} | SHOULDER: {metrics['shoulder_status']}",
            (20, h - 20),
            cv2.FONT_HERSHEY_SIMPLEX,
            0.8,
            (0, 255, 0),
    ...
    def recv(self, frame):
        ...
        result = self._landmarker.detect_for_video(mp_img, self._frame_timestamp_ms)
        ...
        if result.pose_landmarks:
            ...
            if detector:
                ...
                self.set_latest_metric(metrics)
        else:
