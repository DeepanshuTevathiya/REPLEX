...
def sync_metrics_update(context):
    ...
    latest_metrics = processor.get_latest_metric()
    ...
    # DB Operations - adding new sets and time taken to db  
    last_saved_set = st.session_state.get("last_saved_sets_completed", 0)

    if target_sets > 0 and reps_per_set > 0 and sets_completed > last_saved_set:
        newly_completed = sets_completed - last_saved_set
        now_ts = time.time()
